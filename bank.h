...
public:
  ...
  bool is_safe(int id, const ext_vector<int>& req){ 
    ...
    // create copy of bank's available resources using vector Avail
    // vector avail holds set of resources that will be requested by Customers
    ext_vector<int> bank_copy = this->avail;
    // grant request (temporarily to customer) using a copy, subtract req
    bank_copy -= req;

    // go through list of customers, can we get to max?
    for(int x = 0; x < this->avail.size(); ++x){
      if(x == id) 
        continue; // don't count pseudo-approved request
